  ...
  methods: {
	...
	// If it contains
      ...
      if (this.currentBasket.filter((item: Tmp) => item.id === delta.id).length > 0) {
        // Remove from basket
        // @ts-ignore
        this.currentBasket = this.currentBasket.filter(
          (item: Tmp) => item.id !== delta.id
        );
      } else {
        // @ts-ignore
    ...
    <div class="col-md-9">
      ...
      <div class="tab-content" id="myTabContent">
        <Tab :active="true" :id="'welcome'">
          ...
          <p>
            Here you can design your own course from the GTN and Gallantries'
            Library of Video Content. Follow the steps below to build your
            course
          </p>
          <ol>
            ...
            <li>
              Configure the event settings like the title, start and end time,
              etc.
            </li>
            <li>Preview the daily schedule.</li>
            <li>
              And receive a Markdown file that can be contributed back to this
              repository to host your event.
            </li>
          </ol>
        </Tab>
